...
export default function ServicePage() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch("/api/entries?kind=service");
  ...
  return (
    <div className="space-y-7">
      <PageHeader
        strand="Service"
        title="Service reflections"
        description="Weekly service experiences documented with photos and reflections."
      />

      {entries.length === 0 && (
        <div className="rounded-3xl border border-black/5 bg-white/70 p-6 text-sm text-slate-600">
          No service entries yet. Create one in the Admin panel.
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="mt-5 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                ...
                <span className="inline-flex w-fit items-center rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-[0.65rem] uppercase tracking-[0.16em] text-rose-700">
                  service
                </span>
              </div>
